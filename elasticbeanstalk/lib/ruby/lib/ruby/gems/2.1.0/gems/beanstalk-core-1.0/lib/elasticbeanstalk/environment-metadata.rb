...
module ElasticBeanstalk
    class EnvironmentMetadata
        ...
        attr_accessor :environment_id, :environment_bucket, :stack_name, :resource, :region, :cfn_url
        ...
        def instance_id
            @instance_id ||= Kernel.open('http://169.254.169.254/latest/meta-data/instance-id').read
        end
        ...
        def metadata(path:, delimiter: '||')
            ...
            path.split(delimiter).each do |subkey|
                if metadata.has_key?(subkey)
                    ...
                    @logger.error("Path #{path} does not exist in metadata")
